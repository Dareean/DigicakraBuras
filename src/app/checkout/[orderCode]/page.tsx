...
function PendingVerificationState({
  ...
  return (
    <div className="bg-white p-8 rounded-lg border border-slate-200 shadow-sm text-center space-y-6 flex flex-col items-center">
      <div className="p-3 bg-amber-100 text-amber-600 rounded-full flex items-center justify-center animate-pulse">
        <Clock className="w-10 h-10" />
      </div>
      <div>
        <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2.5 py-1 rounded-full border border-amber-100 uppercase tracking-wider">
          Menunggu Verifikasi
        </span>
        <h3 className="text-lg font-bold text-slate-800 mt-3">Pembayaran Sedang Ditinjau</h3>
        <p className="text-xs text-slate-500 mt-1 max-w-xs mx-auto">
          Bukti pembayaran telah kami terima dan sedang diverifikasi oleh staf kami.
        </p>
      </div>

      <div className="w-full pt-4 border-t border-slate-100 space-y-2">
        <p className="text-[10px] text-slate-400 text-center">
          Anda akan menerima notifikasi otomatis setelah pembayaran disetujui.
        </p>
        <button
          type="button"
          onClick={onManualRefresh}
...
function ActivePaymentSelection({
  order,
  qrCodeUrl,
  isRefreshing,
  onManualRefresh,
  ...
  isRefreshing: boolean;
  onManualRefresh: () => void;
  fetchOrderDetails: (silent?: boolean) => Promise<void>;
}) {
  ...
  const handleUploadReceipt = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        const res = await fetch(`/api/orders/${order.orderCode}/manual-payment`, {
          method: "POST",
      ...
      reader.onerror = () => {
        setErrorMsg("Gagal membaca file bukti transfer");
        setUploading(false);
      };
    } catch (err: any) {
      console.error(err);
    ...
    <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col">
      ...
      <div className="grid grid-cols-2 bg-slate-100 p-1 border-b border-slate-200">
        <button
          ...
          className={`py-2 text-xs font-bold rounded text-center transition-all cursor-pointer ${
            paymentTab === "qris"
      ...
      {paymentTab === "qris" ? (
        ...
        <div className="p-6 text-left space-y-5">
          <div className="border-b border-slate-100 pb-3">
            <h3 className="text-base font-bold text-slate-800">Transfer Bank Manual</h3>
            <p className="text-xs text-slate-400 mt-0.5">Transfer persis sesuai total pembayaran, lalu upload bukti transfer Anda.</p>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-2 text-xs">
            <div className="flex justify-between border-b border-slate-100 pb-2">
              ...
              <span className="font-bold text-slate-800">Bank BRI</span>
            </div>
            <div className="flex justify-between border-b border-slate-100 pb-2">
              <span className="text-slate-400 font-medium">Nomor Rekening:</span>
              <span className="font-black text-slate-900 tracking-wider">0023-01-000456-50-2</span>
            </div>
            <div className="flex justify-between border-b border-slate-100 pb-2">
              <span className="text-slate-400 font-medium">Atas Nama:</span>
              <span className="font-bold text-slate-800">Bu Herlina (Fotocopy Cakrawala)</span>
            </div>
            <div className="flex justify-between items-baseline pt-1">
              <span className="text-slate-400 font-medium">Total Nominal:</span>
              <span className="text-sm font-extrabold text-red-600">Rp {order.totalAmount.toLocaleString("id-ID")}</span>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <label
                ...
                className="cursor-pointer py-2 px-3 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 rounded text-xs font-bold transition-all shadow-sm flex items-center gap-1.5"
              >
                📁 Pilih Gambar Bukti
              </label>
              {receiptFile && (
                <span className="text-xs text-slate-550 truncate max-w-[180px] font-medium">
                  {receiptFile.name}
                </span>
              )}
            </div>
            ...
            {/* Receipt Preview */}
            {receiptPreview && (
              <div className="border border-slate-150 rounded p-2 bg-slate-50 flex items-center justify-center max-h-40 overflow-hidden">
                <img
                  src={receiptPreview}
            ...
            {errorMsg && (
              <p className="text-xs text-red-655 font-bold">{errorMsg}</p>
            )}
            ...
            <button
              ...
              className="w-full py-2.5 bg-red-600 hover:bg-red-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white text-xs font-bold rounded shadow transition-all flex items-center justify-center cursor-pointer"
            >
              {uploading ? (
                <>
